      ...
      <div className={`flex flex-col bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl transition-all duration-300 ${
        ...
        <div className="p-6 border-b border-white/5 bg-white/5 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            {quiz && (
               <button onClick={() => setQuiz(null)} className="text-xs text-red-400 hover:text-red-300 uppercase font-bold bg-red-400/10 px-3 py-1 rounded-full">
                 End Quiz
               </button>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-slate-700">
          {!quiz ? (
            ...
            <div className="space-y-8 max-w-4xl mx-auto">
              {quiz.map((q, i) => (
                <div key={q.id} className="animate-fade-in bg-slate-800/30 p-6 rounded-2xl border border-white/5">
                  <h3 className="text-lg md:text-xl text-white font-medium mb-4 flex gap-3">
                        ...
                        <button
                          ...
                          className={`text-left p-4 rounded-xl border transition-all ${
                            answers[q.id] === label 
                              ...
                              : 'bg-slate-800/80 border-white/5 text-slate-300 hover:bg-slate-700 hover:border-white/20'
                          }`}
                        >
